...
export function shapeStageCheckData(rows: any[], courseName: string) {
  ...
  const courseRows = rows
    .filter(r => r.Course?.trim() === courseName?.trim())
    .map(r => ({
      ...r,
...
// similar shaping but for ALL rows, not just stage checks
export function shapeAllLessonsFromStageCheckFile(rows: any[], courseName: string) {
  const courseRows = rows
    .filter(r => r.Course?.trim() === courseName?.trim())
    .map(r => ({
      ...r,
  ...
  return courseRows.map(r => {
    ...
    return {
      student: r.Student,
      courseName,
      stage: r["Stage Check"] || "N/A",
      lessonName: r.Lesson,
      checkInstructor: r.Instructor,
      date: r.parsedDate,
      attempt: Number(r.Attempt) || 1,
      passed: grade === "COMPLETE",
      failed: grade === "UNSATISFACTORY",
      incomplete: grade === "INCOMPLETE",
      continuation: grade === "CONTINUE",
      flightsBeforeStage: courseRows.filter(l =>
        l.Student === r.Student && l.parsedDate < r.parsedDate
      ).length
    };
  });
}
